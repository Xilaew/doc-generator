...
def generateRandomMarkdownTable (outputStream):
	...
	for x in range(width):
		outputStream.write(''.join([random.choice(string.ascii_letters + string.digits) for n in range(random.randint(2,5))]))
		outputStream.write('|')
	outputStream.write('\n|')
	for x in range(width):
		outputStream.write('---|')
	for x in range(height):
		...
		for y in range(width):
			outputStream.write(''.join([random.choice(string.ascii_letters + string.digits) for n in range(random.randint(2,20))]))
			outputStream.write('|')
